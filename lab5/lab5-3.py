"""
Функции - это удобный способ разделить ваш код на полезные блоки, сделать его более читабельным
и помочь в его повторном использовании. Функции определяются с помощью ключевого слова def,
...
def fun():
    print("Слово - серебро, а молчание - немота.")

# Это основная программа (Напишите эту заметку при написании основной
# программы)
...
# замените дубликаты строк в файле (реализуйте подпрограмму, которая
...
# Определите функцию с именем square, печатающую квадрат передаваемого ей
# значения.
...
def fib(n):
    ...
    while a < n:
        ...
        b += a
        # обновите значение переменной a, поместив в нее значение
